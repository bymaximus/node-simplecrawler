...
Crawler.prototype.fetchQueueItem = function(queueItem) {
    ...
    crawler.queue.update(queueItem.id, {
        ...
        clientRequest.on("aborted", function(errorData) {
        	//console.log('REQUEST ABORTED');
        });
        clientRequest.on("connect", function(errorData) {
            //console.log('REQUEST CONNECT');
        });
        clientRequest.on("continue", function(errorData) {
            //console.log('REQUEST CONTINUE');
        });
        clientRequest.on("socket", function(errorData) {
            //console.log('REQUEST SOCKET');
        });
        clientRequest.on("upgrade", function(errorData) {
    ...
    crawler.queue.update(queueItem.id, {
        ...
        // If we should queue a redirect
            ...
            crawler.queue.update(queueItem.id, {
                ...
                if (redirectQueueItem.protocol === "https") {
                    redirectQueueItem.port = 443;
                }				
